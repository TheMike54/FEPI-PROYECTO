-- =====================================================================
-- SEED LOCAL para smoke de HU-13 (Equipo 3) — NO es parte de schema.sql.
-- Script aparte, idempotente y re-ejecutable. Crea lo MÍNIMO para ENVIAR una
-- estimación: 1 contrato con equipo (incl. superintendente) + 1 estimación
-- 'integrada' (enviable) con su generador.
--   · EST-001 (numero 1): periodo Ene 2026, estado 'integrada', neto 34750.00
--
-- A diferencia del seed de HU-14, este RESETEA la estimación a 'integrada' si ya
-- existe (limpiando el sello enviada_en/por). Así el smoke de HU-13 (que ENVÍA y muta
-- el estado a 'enviada') es re-ejecutable aunque una corrida previa haya quedado a
-- medias: el precondición "hay una estimación enviable" se restablece. El trigger
-- sigecop_estimacion_inmutable congela SOLO la carátula y deja libre estado/enviada_*,
-- así que este UPDATE de estado/sellos es seguro (no toca la carátula).
--
-- Reusa cuentas DEMO existentes (no crea credenciales):
--   · created_by / residente_id = 1  (Ing. Residente Demo)
--   · superintendente_id        = 2  (Contratista Demo S.A.) — el superintendente es
--                                     una cuenta rol 'contratista' (regla HU-12/HU-13:
--                                     solo él envía sus estimaciones)
--   · supervision_id            = 3  (Supervisión Externa Demo)
--   · integrada_por             = 2  (el superintendente integra)
--
-- Carátula consistente con la fórmula server-side de HU-12 (anticipo 30%, 5 al millar):
--   subtotal     = ROUND(50 × 1000, 2)       = 50000.00
--   amortización = ROUND(50000 × 30/100, 2)  = 15000.00
--   retención    = ROUND(50000 × 0.005, 2)   =   250.00   (art. 191 LFD)
...
BEGIN
  ...
  SELECT id INTO v_contrato_id FROM contratos WHERE folio = 'SMOKE-HU13-001';
  IF v_contrato_id IS NULL THEN
    ...
    VALUES ('SMOKE-HU13-001', 'Obra pública', 'Contrato semilla para smoke HU-13',
  ...
  IF v_concepto_id IS NULL THEN
    ...
    VALUES (v_contrato_id, 1, 'Concepto semilla (smoke HU-13)', 'm3', 100.000, 1000.00)
    RETURNING id INTO v_concepto_id;
  END IF;

  -- (3) Estimación enviable (idempotente por contrato_id + numero). Si NO existe, se crea
  --     'integrada'; si YA existe, se RESETEA a 'integrada' limpiando el sello (re-run).
  ...
  ELSE
    -- Restablece la precondición del smoke: enviable de nuevo (no toca la carátula).
    UPDATE estimaciones
       SET estado = 'integrada', enviada_en = NULL, enviada_por = NULL
     WHERE id = v_estimacion_id;
  END IF;

  RAISE NOTICE 'SEED HU-13 listo: contrato_id=%, concepto_id=%, estimacion_id=%',
...
SELECT c.id AS contrato_id, c.folio, c.residente_id, c.superintendente_id, c.supervision_id,
       e.id AS estimacion_id, e.numero, e.estado, e.neto, e.enviada_en
  FROM contratos c
  JOIN estimaciones e ON e.contrato_id = c.id
 WHERE c.folio = 'SMOKE-HU13-001'
 ORDER BY e.numero;
