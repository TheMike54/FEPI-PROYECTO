...
// Umbrales del semáforo del plazo de pago — DEFAULTS PROVISIONALES. [validar profe].
const PLAZO_PAGO_DIAS = 20;                 // art. 54/55 LOPSRM (20 días naturales)
const SEMAFORO = { verde_max: 10, ambar_max: 17 }; // verde ≤10 · ámbar 11-17 · rojo >17
...
async function generarInstruccion(req, res) {
  try {
    ...
    // CA-1: suficiencia presupuestal (art. 24).
    const ejercicio = ejercicioDe(est.fecha_inicio);
    const suf = await calcularSuficiencia(est.dependencia, ejercicio, est.id, est.neto);
    if (suf.sin_presupuesto) {
      return res.status(409).json({ error: `No hay techo presupuestal cargado para (${est.dependencia || 's/dependencia'}, ${ejercicio || 's/ejercicio'}); no se puede verificar la suficiencia (art. 24).`, suficiencia: suf });
    }
    if (suf.excede) {
      return res.status(409).json({ error: `El neto ($${suf.neto}) excede el disponible ($${suf.disponible_antes}); requiere ampliación presupuestal (art. 24 LOPSRM).`, suficiencia: suf });
    }

    // CA-3: soportes obligatorios.
    ...
    try {
      const ins = await pool.query(
        `INSERT INTO instruccion_pago
         ...
         RETURNING *`,
        [est.id, suf.presupuesto_id, r2(est.neto), sop.folio_cfdi, req.user.id]
      );
      return res.status(201).json({ ok: true, instruccion: ins.rows[0], suficiencia: suf });
    } catch (e) {
