...
- **Roles del sistema** (cuentas demo, contraseña `Sigecop2026!`): residente@, contratista@ (superintendente), supervision@, dependencia@, finanzas@ — todos @sigecop.test — y la cuenta del profe (csilvasa@ipn.mx).
...
5. **HU-03 (Maiki la tomó):** Fase 1 = convenios de plazo con historial inmutable y avisos 25/50; Fase 2 = editor de matriz para convenios de monto/programa/mixto (deriva el monto al centavo, versiona el programa).
6. **Hoy:** ~17 HU funcionando end-to-end en Render. Quedan de los equipos: HU-11 + correcciones estructurales de bitácora (E2) y HU-15–20 (E3). El profe revisó el 8–9 de junio: este documento consolida esa revisión.
...
2. **Estados vacíos guiados:** cada vista sin datos ofrece el siguiente paso con botón (sin bitácora → "Abrir bitácora"; sin estimaciones → "Presentar estimación"; sin contratos → "Registrar contrato"), respetando permisos (el CTA solo aparece si el rol puede ejecutarlo).
3. **Micro-UX (UI-B):** toasts de éxito/error con un solo componente; validación inline junto al campo; skeletons de carga; helper único de formato es-MX para moneda y fechas.
4. **Regla dura:** conservar **todos los `data-testid`** (la suite no se rompe); si cambian selectores de navegación del sidebar, actualizar los helpers de los specs (lección 7). Contraste AA del guinda con blanco; foco visible.
...
### Oleada UI — Reskin "institucional moderno" (B + paleta guinda) — **PRIORIDAD AHORA** (revisión movida al lunes)
**Decisión de diseño (Maiki):** estructura limpia de SaaS (sidebar claro, cards blancas, badges suaves, mucho aire, buscador en el header) con **identidad institucional**: **barra superior guinda** (#691C32) con filo **dorado** (#BC955C) y sidebar claro con estados activos en guinda. Da el sello "obra pública" sin perder el look de producto.

**REGLA DE ORO — es SOLO reskin:** no se toca lógica, endpoints, cálculos (G1–G8), validaciones, `permisos.js`, `server.js` ni llamadas a la API. Solo markup JSX + clases Tailwind + componentes compartidos nuevos. **Se conservan TODOS los `data-testid` y los textos que la suite asercióna** (labels, "solo consulta", textos de botón) → la suite e2e es la red de seguridad y debe quedar verde.

**Tokens (extender `tailwind.config.js`):** `guinda #691C32`, `guinda-dark #4E1525`, `guinda-soft #F3E9EC`, `dorado #BC955C` (filo/acentos, NO texto sobre blanco); texto `#2C2C2A`/secundario `#888780`/terciario `#B4B2A9`; página `#FAFAF8`, superficie `#FFFFFF`, sidebar `#FAFAF8`; borde `#ECEAE4`/fuerte `#E2DFD8`; estados éxito `#EAF3DE/#3B6D11`, aviso `#FAEEDA/#854F0B`, peligro `#FCEBEB/#A32D2D`, info/primario `#F3E9EC/#691C32`; radius md 8 / lg 12; tipografía Inter (o system sans), pesos 400/500, sentence case.

**Componentes compartidos (`frontend/src/components/ui/`):** `AppShell` (barra guinda con filo dorado: logo + buscador + campana + chip de rol + avatar; sidebar claro con item activo guinda; contenido sobre `#FAFAF8`; reemplaza el layout actual SIN tocar el router) · `Card` · `Kpi` · `Badge` · `Tabla` · `EncabezadoContrato` · `Boton` (primario guinda / secundario outline) · `Modal` (reusa el de la garantía) · `BannerSoloConsulta`.

**Orden de migración (lo que el profe ve más, primero):**
- **UI-1 (fundación + alto valor):** tokens + AppShell + componentes + Login + Alta + Bitácora (notas/firmas) + Expediente + Estimación → reportar, Maiki revisa e integra.
- **UI-2 (resto):** Curva de avance, Tablero, Convenios, Alertas, Avance físico, Pago, Historial, Sustitución/Roster, Detalle de contrato.

**Prompt UI-1:**
```
Soy Maiki. RESKIN de la UI — "institucional moderno" (estructura limpia + paleta guinda institucional). PARTE 1: fundación + páginas de alto valor. LOCAL, sin commit/push, sobre main actual.
REGLA DE ORO: SOLO reskin. NO toques lógica, endpoints, cálculos (G1-G8), validaciones, permisos.js, server.js ni llamadas a la API. Solo markup JSX + clases Tailwind + componentes nuevos. CONSERVA todos los data-testid y los textos que la suite asercióna (labels, "solo consulta", textos de botón). La suite e2e debe seguir verde.
1) tailwind.config.js: extiende colores guinda #691C32, guinda-dark #4E1525, guinda-soft #F3E9EC, dorado #BC955C; estados éxito #EAF3DE/#3B6D11, aviso #FAEEDA/#854F0B, peligro #FCEBEB/#A32D2D, info #F3E9EC/#691C32. Tipografía Inter/system sans, pesos 400/500, sentence case. Radius md 8 / lg 12.
2) Componentes en frontend/src/components/ui/: AppShell (barra superior guinda con filo dorado 3px: logo + buscador + campana + chip de rol + avatar; sidebar CLARO #FAFAF8 con item activo guinda-soft + texto guinda + borde-izq guinda; contenido sobre #FAFAF8), Card, Kpi, Badge, Tabla, EncabezadoContrato, Boton (primario guinda / secundario outline), BannerSoloConsulta. Reemplaza el layout/sidebar actual con AppShell SIN tocar el router ni las rutas.
3) Migra SIN cambiar comportamiento: Login (pantalla previa al shell, limpia, identidad guinda), Alta de contrato, Bitácora (notas + firmas), Consulta de expediente, Estimación (pantalla única).
4) Corre la suite completa. Si un test truena por un texto que renombraste, conserva el texto original; si el cambio es intencional, actualiza SOLO ese spec (lección 7). Reporta passed/skipped/failed.
Doc + runbook (capturas antes/después si puedes). NO push.
```

**Prompt UI-2:**
```
Soy Maiki. RESKIN UI PARTE 2 — migrar el resto de páginas al sistema de diseño de UI-1 (mismos componentes y tokens guinda). LOCAL, sin commit/push, sobre main actual (con UI-1 ya integrada).
Misma REGLA DE ORO: solo reskin, sin tocar lógica/endpoints/cálculos/permisos/server; conserva data-testid y textos asercionados; suite verde.
Migra a los componentes compartidos: Curva de avance, Tablero de estimaciones, Convenios (lista + editor de matriz), Alertas/Atrasos, Registro de avance físico, Pago, Historial de estimaciones, Sustitución/Roster, Detalle de contrato.
Corre la suite completa. Reporta. Doc + runbook. NO push.
```
...
5. **Folio diferido** cuando no hay bitácora abierta: ¿satisface art. 123 fr. V/VI?
6. **Alertas:** ¿"marcar visto"/silenciar es aceptable o el déficit siempre visible?
7. **Misma empresa contratista–supervisión:** ¿aviso o bloqueo?
8. **Flujo legal de la estimación (S5):** confirmar el mapeo contratista-presenta / residente-autoriza antes de construir (o se construye por art. 54).
9. (Anteriores que siguen: IVA, regla 100% exacta, dependencia-no-firma, anular nota ya-conforme, Obra+tablero por definir, login-contrato UX.)
...
**Calendario sugerido (revisión del profe movida al LUNES):** HOY: O0 (backup real + decisión A/B) + revisar/integrar/pushear O1 + lanzar **Oleada UI-1** (fundación + páginas de alto valor). **Mié–vie:** integrar UI-1, lanzar **UI-2** (resto de páginas) + O2 (plan de amortización). **Fin de semana:** O3 (empresas), O4 (avance por periodo). **LUNES (revisión):** mostrar O1 + UI nueva, validar S5 (flujo legal) y los `[validar]` Top-3 (amortización en carátula, 25%, exceso de periodo); demo en vivo de la fecha pasada (P2). **Después:** O5, O6, O7, O8, O9 + integrar PRs de E2/E3. **23–24:** migración/upgrade de BD + smoke E2E completo con la guía. **25–28:** colchón y entrega.
