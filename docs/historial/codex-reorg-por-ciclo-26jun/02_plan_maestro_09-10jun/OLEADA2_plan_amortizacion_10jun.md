...
> Ley: el anticipo se amortiza con cargo a las estimaciones (**art. 138 fr. I RLOPSRM**, que indica
...
2. **Backend** (`contratos.controller.js`, bloque aditivo en `crearContrato`, transaccional):
   - Valida el plan recibido: periodos en rango, sin repetidos, montos ≥ 0 y **Σ = ROUND(monto × %anticipo, 2) EXACTA al centavo** → si no, **400 citando el art. 138**.
...
6. **G2 intacto**: ni una línea de `estimaciones.controller.js`. La amortización de la carátula sigue siendo proporcional hasta el OK del profe (el art. 138 dice "proporcionalmente"; que él decida si el sistema obedece al plan — pregunta #1 de la lista [validar]).

## Tests (lección 7)

- **Nuevo** `o2-plan-amortizacion.spec.js` (4 escenarios): default proporcional cuadra / editar descuadra y bloquea / restablecer desbloquea · plan editado (todo en P1) se **persiste tal cual** (verificado por API) · sin anticipo la pestaña no existe y Siguiente/Atrás saltan el paso · API sin plan deriva proporcional + API con plan descuadrado → 400 con art. 138.
...
4. **Para la clase**: mostrar P6 resuelto y plantear la pregunta #1 de la lista [validar]: ¿la carátula debe obedecer al plan capturado (Fase B) o mantenerse proporcional (art. 138 literal)?
