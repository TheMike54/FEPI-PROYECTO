...
- **Deseo** seleccionar una estimación AUTORIZADA y ver/gestionar su tránsito a pago contra el backend real: (1) verificación de suficiencia presupuestal server-side (techo de `presupuesto_anual` − comprometido) que BLOQUEA la instrucción si el neto excede lo disponible (art. 24); (2) un semáforo del plazo de 20 días anclado en la fecha real de autorización (derivada de la nota de bitácora `res_estimaciones`); (3) un checklist de soportes (factura/CFDI por metadato + fianza de cumplimiento leída de garantías) que condiciona la generación; y un botón que genera la instrucción de pago real (persistida en `instruccion_pago`, una por estimación) y notifica a Finanzas
- **A fin de** ejecutar el tránsito a pago con los controles legales reales (art. 24 y art. 54 LOPSRM) cuadrados al centavo, sin dummy

**Implementación (cableada al backend, rama `feat/e3-hu-20`, pendiente de montaje en `server.js` por Maiki):** controller+route NUEVOS `instruccion-pago.controller.js`/`.routes.js` (dominio E3, acotado por participación `lib/acceso.js`). Endpoints: `GET /api/instruccion-pago/estimacion/:id` (estado de tránsito), `POST .../estimacion/:id` (genera instrucción), `POST .../estimacion/:id/soportes` (metadatos), `GET/POST /api/instruccion-pago/presupuesto` (techo; POST = finanzas). `TransitoPago.jsx` reescrita a datos reales (componentes ui/).

**Criterios de aceptación (comportamiento actual del sistema):**
1. **CA-1 (art. 24):** el backend calcula `disponible = techo(presupuesto_anual por dependencia+ejercicio) − comprometido (Σ neto de estimaciones 'autorizada'+'pagada' de esa dependencia/ejercicio, excluyendo la actual)` y BLOQUEA con 409 la generación si `neto > disponible`. Si no hay techo cargado → 409 "sin presupuesto" (no se inventa). El techo lo carga finanzas (`POST /presupuesto`, upsert por (ejercicio, dependencia)).
2. **CA-2 (art. 54):** semáforo de 20 días anclado en la fecha REAL de autorización = `MIN(bitacora_notas.fecha)` de la nota `tipo='res_estimaciones', tag='estimacion'` ligada por `estimacion_notas`. Umbrales provisionales verde ≤10 / ámbar 11-17 / rojo >17 [validar profe]; avisa al entrar en ámbar. Si el contrato no tenía bitácora al autorizar (no hay nota) → semáforo DESHABILITADO con etiqueta (sin inventar); el ancla canónica requiere el sello `autorizada_en` (PARA MAIKI).
3. **CA-3:** la instrucción solo se genera con soportes obligatorios OK: factura (metadato) + CFDI con folio (metadato → `instruccion_pago.factura_cfdi`) + fianza de cumplimiento vigente leída de `contrato_garantias` (tipo cumplimiento). "Cuando el contrato lo exija" = hay garantía de cumplimiento registrada [validar profe]. La **subida de archivos binarios está DESHABILITADA con etiqueta** (sin infra de almacenamiento; no se agregó multer/BYTEA).
4. La generación exige estimación `'autorizada'` (server-side), persiste en `instruccion_pago` (estado `'emitida'`, `monto = neto` snapshot ROUND al centavo, `notificado_finanzas_en` sellado) y es idempotente: `UNIQUE(estimacion_id)` → segunda generación 409.
5. La HU respeta la matriz de acceso: contratista/finanzas ejecutan; residente/dependencia ven en solo lectura; supervisión no ve la HU.

**Fundamento legal:** art. 24 LOPSRM (suficiencia presupuestaria); art. 54 LOPSRM (plazo de pago de 20 días naturales). art. 55 (gastos financieros por mora) NO implementado.

**Pendientes / [validar profe]:**
- 🔴 **PARA MAIKI:** montar la ruta en `server.js` (snippet "PARA MAIKI" en `instruccion-pago.routes.js`). Ancla canónica de CA-2: agregar `autorizada_en`/`autorizada_por` a `estimaciones` y setearlo en `autorizarEstimacion` (esquema + core congelados); mientras tanto el ancla es la nota de bitácora (derivada).
- [validar profe] definición de "comprometido" (Σ neto autorizadas+pagadas); join contrato→presupuesto por dependencia (texto) + ejercicio (year de fecha_inicio), sin FK (frágil).
- [validar profe] umbrales del semáforo; "cuando el contrato lo exija" para la fianza (¿siempre, art. 48?); quién genera la instrucción (contratista vs finanzas; hoy ambos); si la carga del techo (`POST /presupuesto`) es parte de HU-20 o un concern aparte.
- Subida de ARCHIVOS de soportes (factura/CFDI/fianza): AUSENTE (sin BYTEA ni multer genérico) — se registran metadatos; flagueado, no agregado en silencio.
- Notificación a Finanzas: sello `notificado_finanzas_en` (in-app Etapa 1); no hay correo real.
