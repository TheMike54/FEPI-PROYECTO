...
- **Deseo** deseo que al registrarme capture mi empresa con autocompletado contra un catalogo unico (si no existe, se da de alta sola tras confirmar; si existe, la TOMA sin duplicar), que el sistema deduplique por nombre normalizado FUERTE (mayusculas/espacios + acentos + puntuacion + sufijos de razon social tipo 'SA de CV' — FASE 3 15-jun), que el alta de contrato me AVISE (sin bloquear) cuando superintendente y supervision son de la misma empresa, y que el expediente muestre la empresa de cada persona y permita buscar por empresa
...
2. Registro con empresa EXISTENTE: al elegir/teclear una empresa que ya esta en el catalogo NO se vuelve a preguntar y NO se duplica. La deteccion es por match FUERTE (FASE 3 15-jun): incluso una VARIANTE con acentos, puntuacion o sufijo de razon social ('Constructora Demo', 'Constructóra Demo' o 'Constructora Demo, S.A. de C.V.') resuelve a la MISMA empresa — el backend (resolverOCrearEmpresa, normalizarNombreEmpresaFuerte) la reutiliza y el front avisa 'se usara la empresa ya registrada' (data-testid reg-empresa-existente). El indice unico debil uq_empresas_nombre_norm queda como respaldo. Probado en o3-empresas.spec.js (empresa EXISTENTE; variante con acento+sufijo reutiliza; aviso UI).
