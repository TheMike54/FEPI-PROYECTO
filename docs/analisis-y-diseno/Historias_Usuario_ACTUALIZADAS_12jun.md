...
- **Deseo** deseo que al registrarme capture mi empresa con autocompletado contra un catalogo unico (si no existe, se da de alta sola tras confirmar; si existe, la elijo sin duplicar), que el sistema deduplique por nombre normalizado (mayusculas/espacios), que el alta de contrato me AVISE (sin bloquear) cuando superintendente y supervision son de la misma empresa, y que el expediente muestre la empresa de cada persona y permita buscar por empresa
...
**Criterios de aceptación (comportamiento actual del sistema):**
1. Registro con empresa NUEVA: al teclear una empresa que no esta en el catalogo (comparacion normalizada lower+trim+espacios colapsados), el formulario pide confirmacion ('X no esta en el catalogo. Registrarla como nueva empresa?') y al aceptar + enviar la empresa queda dada de alta y aparece en GET /api/auth/empresas (alta automatica). Probado en o3-empresas.spec.js (registro empresa NUEVA).
2. Registro con empresa EXISTENTE: al elegir/teclear una empresa que ya esta en el catalogo NO se vuelve a preguntar y NO se duplica (el INSERT que colisionaria por uq_empresas_nombre_norm reutiliza el id existente). Probado en o3-empresas.spec.js (empresa EXISTENTE no re-pregunta ni duplica).
