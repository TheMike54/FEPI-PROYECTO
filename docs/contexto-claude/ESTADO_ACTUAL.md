...
empresas), **~22 funcionan end-to-end** (≈85%); **HU-11 minutas** sigue maqueta; **HU-18 portafolio** y
**HU-20 tránsito a pago** ya están cableadas a backend real en sus ramas de Equipo 3 (controller+route+página
nuevos) pero **pendientes de montaje en `server.js` por Maiki** (aún NO en `main`); **HU-02 fianzas** es
...
| **HU-18** Portafolio | `PortafolioEjecutivo.jsx` | 5 contratos hardcodeados; el semáforo se calcula pero sobre dummy; no filtra por usuario | Falta endpoint + filtro por participación (cableado en rama `feat/e3-hu-18`, pendiente de montaje) |
